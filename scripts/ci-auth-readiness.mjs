export function ciAuthReadiness(env = process.env) {
  ...
  if (hasShortLived) return { mode: 'short_lived_access_token', ready: true, migration_ready: true, legacy_fallback_present: hasRefreshToken };
  if (hasWifCreds) return { mode: 'wif_adc_credentials_present', ready: false, migration_ready: true, legacy_fallback_present: hasRefreshToken };
  if (managedRefreshReady) return { mode: 'managed_firebase_refresh_token', ready: true, migration_ready: false, legacy_fallback_present: true };
  if (hasRefreshToken) return { mode: 'refresh_token_missing_oauth_client_credentials', ready: false, migration_ready: false, legacy_fallback_present: true };
