...
for (const token of ['Base legal','Plazo de retención','Evento de inicio del plazo','Excepciones / litigio / fraude','Responsable de aprobar']) assert.match(doc, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
for (const collection of ['users','user_private','device_tokens','listings_v2','demand_requests','transactions_v2','offers','chats','reviews','reports','audit_log','account_deletion_requests']) assert.match(doc, new RegExp(`\\`${collection}\\``));
