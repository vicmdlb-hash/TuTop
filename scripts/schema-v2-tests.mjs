...
assert.match(rules, /function sellerAcceptanceReady\(offerId, offerData\)/);
assert.match(rules, /let txId = 'tx-' \+ offerId/);
assert.match(rules, /existsAfter\(\/databases\/\$\(database\)\/documents\/transactions_v2\/\$\(txId\)\)/);
assert.match(rules, /getAfter\(\/databases\/\$\(database\)\/documents\/products\/\$\(offerData\.listing_id\)\)\.data\.estado == 'Reservado'/);
assert.match(rules, /sellerAcceptanceReady\(offerId, resource\.data\)/);
...
assert.match(nationalBackend, /finalizeCompletedListing/);
assert.match(nationalBackend, /transaction\.status !== 'completed' \|\| !transaction\.buyer_confirmed_at \|\| !transaction\.seller_confirmed_at/);
...
console.log('PASS seller can finalize completed listing after either confirmation order');
