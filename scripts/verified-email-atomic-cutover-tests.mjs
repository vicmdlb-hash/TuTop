...
function fixture(){let current='old';const writes=[];const api={billing:async()=>({billingEnabled:false}),release:async()=>({rulesetName:current}),ruleset:async()=>({source:{files:[{content:'old rules'}]}}),createRuleset:async()=>({name:'new'}),patch:async(name)=>{writes.push(name);current=name;},verify:async()=>{}};return {api,writes,get current(){return current;},set current(v){current=v;}};}
const run=(f,extra={})=>promoteVerifiedRules({api:f.api,source,expectedRuleset:'old',validated:true,...extra});
let tests=0;const t=async(name,fn)=>{await fn();tests++;console.log('PASS '+name);};
