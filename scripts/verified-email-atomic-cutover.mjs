...
export async function promoteVerifiedRules({api,project=STAGING,source,expectedRuleset,apply=false,deviceInstalled=false,exclusiveWindow=false,validated=false,record=()=>{}}){
 ...
 let attempted=false;
 try{attempted=true;await api.patch(candidate.name);const after=await api.release();if(after.rulesetName!==candidate.name)throw new Error('RELEASE_READBACK_MISMATCH');await api.verify();record({phase:'verified',candidate});return {status:'BACKEND_RELEASE_VERIFIED_DEVICE_EVIDENCE_PENDING',candidate:candidate.name,rollback:original.rulesetName};}
 catch(error){if(attempted){const current=await api.release();if(current.rulesetName===candidate.name){await api.patch(original.rulesetName);const restored=await api.release();if(restored.rulesetName!==original.rulesetName)throw new Error('ROLLBACK_READBACK_FAILED');record({phase:'rolled_back',original});}else if(current.rulesetName!==original.rulesetName)throw new Error('CONCURRENT_RELEASE_REQUIRES_REVIEW');}throw error;}
}
