...
export default function AccountDeletionQueue() {
  ...
  const transition = async (request: RequestDoc, status: 'processing' | 'completed' | 'rejected') => {
    const uid = String(request.data.uid || request.id);
    if (!uid || busy) return;
    if (status === 'completed') {
      const confirmed = window.confirm('Marca como completada sólo cuando el borrado/retención aplicable haya sido procesado fuera de esta pantalla. Esta acción NO borra datos por sí misma. ¿Continuar?');
      if (!confirmed) return;
    }
  ...
  return <section className="bg-[#070b12] px-4 pt-4 text-slate-100 sm:px-7 sm:pt-7">
    <div className="mx-auto max-w-7xl rounded-3xl border border-rose-400/10 bg-[#0b111c] p-4 sm:p-5">
      <div className="flex items-center gap-3"><ShieldAlert className="h-5 w-5 text-rose-300"/><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.16em] text-rose-300">Privacidad / cuenta</p><h2 className="text-lg font-black">Solicitudes de eliminación</h2><p className="text-[10px] leading-4 text-slate-500">Esta cola gestiona estados y auditoría. No ejecuta un borrado destructivo automático ni permite marcar “completada” sin confirmación explícita del operador.</p></div><button onClick={() => void refresh()} className="grid h-9 w-9 place-items-center rounded-xl border border-white/[0.07] bg-white/[0.035] text-slate-400" aria-label="Actualizar solicitudes"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`}/></button></div>
