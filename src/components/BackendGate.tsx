    ...
    <div className="auth-screen min-h-screen px-4 py-8 text-white sm:grid sm:place-items-center">
      <div className="w-full max-w-md overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#0b111c] shadow-2xl">
        ...
        <div className="border-t border-white/[0.06] p-5 sm:p-6">
          <div className="grid grid-cols-3 rounded-xl bg-[#070b12] p-1"><button onClick={() => { setMode('register'); setStep(1); setError(null); }} className={`rounded-lg py-2.5 text-[10px] font-black ${mode === 'register' ? 'bg-violet-600 text-white' : 'text-slate-500'}`}>Crear cuenta</button><button onClick={() => { setMode('login'); setStep(1); setError(null); }} className={`rounded-lg py-2.5 text-[10px] font-black ${mode === 'login' ? 'bg-violet-600 text-white' : 'text-slate-500'}`}>Entrar</button><button onClick={() => { setMode('legacy'); setStep(1); setError(null); }} className={`rounded-lg py-2.5 text-[10px] font-black ${mode === 'legacy' ? 'bg-violet-600 text-white' : 'text-slate-500'}`}>Cuenta anterior</button></div>
          ...
          {mode === 'legacy' && <>
            <div className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/[0.05] p-3 text-[10px] leading-5 text-amber-100/80">Usa el número y la Clave TuTop de tu cuenta anterior. El número sólo sirve para localizar esa cuenta; no se considera verificado. Tu UID, publicaciones, chats y UCoins se conservan.</div>
