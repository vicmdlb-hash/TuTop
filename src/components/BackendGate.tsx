...
import { AlertTriangle, ArrowLeft, ArrowRight, Eye, EyeOff, Loader2, LockKeyhole, Phone, ShieldCheck, Sparkles, UserRound } from 'lucide-react';
...
export default function BackendGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => { window.clearInterval(timer); window.removeEventListener('online', online); document.removeEventListener('visibilitychange', visible); };
  }, [authenticated]);
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  const submit = async () => {
    try {
      ...
      setError(raw === 'Escribe tu nombre.' ? raw : friendlyError(raw));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="auth-screen min-h-screen px-4 py-8 text-white sm:grid sm:place-items-center">
      <div className="w-full max-w-md overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#0b111c] shadow-2xl">
        <div className="relative overflow-hidden px-6 pb-6 pt-7"><div className="absolute inset-0 bg-[radial-gradient(circle_at_10%_0%,rgba(168,85,247,.30),transparent_43%),radial-gradient(circle_at_92%_10%,rgba(56,189,248,.12),transparent_30%)]" /><div className="relative"><div className="flex items-center gap-3"><div className="brand-mark"><span>T</span><i /></div><div><div className="wordmark text-3xl"><span>Tu</span><span>Top</span></div><p className="mt-1 text-[10px] font-bold uppercase tracking-[.18em] text-violet-300/80">Marketplace universitario</p></div></div><h1 className="mt-7 text-[27px] font-black leading-[1.05] tracking-[-.04em]">Compra, vende y conecta sin complicarte.</h1><p className="mt-3 max-w-sm text-sm leading-6 text-slate-400">Explora rápido, publica a tu manera y acuerda entregas con personas de tu comunidad.</p><div className="mt-5 flex flex-wrap gap-2 text-[10px] font-semibold text-slate-300"><span className="auth-benefit"><ShieldCheck />Comunidad protegida</span><span className="auth-benefit"><Sparkles />10 UCoins de bienvenida</span></div></div></div>

        <div className="border-t border-white/[0.06] p-5 sm:p-6">
          <div className="grid grid-cols-2 rounded-xl bg-[#070b12] p-1"><button onClick={() => { setMode('register'); setStep(1); setError(null); }} className={`rounded-lg py-2.5 text-xs font-black ${mode === 'register' ? 'bg-violet-600 text-white shadow-lg shadow-violet-950/30' : 'text-slate-500'}`}>Crear cuenta</button><button onClick={() => { setMode('login'); setStep(1); setError(null); }} className={`rounded-lg py-2.5 text-xs font-black ${mode === 'login' ? 'bg-violet-600 text-white shadow-lg shadow-violet-950/30' : 'text-slate-500'}`}>Entrar</button></div>

          {mode === 'register' && <div className="mt-4"><div className="flex items-center justify-between text-[10px] font-bold"><span className="text-violet-300">Paso {step} de 2</span><span className="text-slate-600">{step === 1 ? 'Tu cuenta' : 'Tu comunidad'}</span></div><div className="mt-2 grid grid-cols-2 gap-1"><span className="h-1 rounded-full bg-violet-500" /><span className={`h-1 rounded-full ${step === 2 ? 'bg-violet-500' : 'bg-white/5'}`} /></div></div>}
          ...
          {mode === 'register' && step === 1 && <>
            ...
            <div className="mt-2 grid grid-cols-3 gap-1">{[1, 2, 3].map((value) => <span key={value} className={`h-1 rounded-full ${passwordScore >= value ? 'bg-emerald-400' : 'bg-white/5'}`} />)}</div><p className="mt-2 text-[9px] text-slate-600">Tu número identifica tu cuenta y nunca aparece públicamente en tus anuncios.</p>
          ...
          {mode === 'register' && step === 2 && <>
            <button onClick={() => setStep(1)} className="mt-4 inline-flex items-center gap-1 text-[10px] font-bold text-slate-500"><ArrowLeft className="h-3.5 w-3.5" />Atrás</button><h2 className="mt-4 text-lg font-black">¿Dónde estudias?</h2><p className="mt-1 text-xs leading-5 text-slate-500">Esto ayuda a mostrarte publicaciones cercanas. Después podrás cambiarlo desde tu perfil.</p><label className="auth-label">Facultad</label><select value={faculty} onChange={(event) => setFaculty(event.target.value)} className="auth-input">{FACULTADES.map((item) => <option key={item}>{item}</option>)}</select>{error && <p className="mt-3 rounded-xl border border-rose-400/10 bg-rose-500/10 px-3 py-2 text-xs text-rose-200">{error}</p>}<button disabled={busy} onClick={() => void submit()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 py-3.5 text-sm font-black disabled:opacity-40">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}Crear mi cuenta TuTop</button><p className="mt-3 text-center text-[9px] leading-5 text-slate-600">La verificación estudiantil es opcional y se hace después. Puedes explorar TuTop desde el primer momento.</p>
          </>}

          {mode === 'login' && <>
            <label className="auth-label">Número celular</label><div className="relative"><Phone className="absolute left-3 top-3.5 h-4 w-4 text-slate-600" /><input value={phone} onChange={(event) => setPhone(event.target.value)} className="auth-input pl-10" inputMode="tel" autoComplete="tel" placeholder="246 123 4567" /></div><label className="auth-label">Clave TuTop</label><div className="relative"><LockKeyhole className="absolute left-3 top-3.5 h-4 w-4 text-slate-600" /><input value={password} onChange={(event) => setPassword(event.target.value)} className="auth-input px-10" type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Tu clave" onKeyDown={(event) => event.key === 'Enter' && void submit()} /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-3.5 text-slate-600" aria-label={showPassword ? 'Ocultar clave' : 'Mostrar clave'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>{error && <p className="mt-3 rounded-xl border border-rose-400/10 bg-rose-500/10 px-3 py-2 text-xs text-rose-200">{error}</p>}<button disabled={busy || !phone.trim() || password.length < 8} onClick={() => void submit()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 py-3.5 text-sm font-black shadow-lg shadow-violet-950/30 disabled:opacity-40">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}Entrar a TuTop</button>
          </>}
        </div>
      </div>
    </div>
