...
export default function Chatbot() {
  ...
  const selectCategory = (category: ProductCategory) => {
    setDraft((current) => ({ ...current, categoria: category }));
    setAdaptive({});
    setError(null);
  };
  ...
  const openPreview = () => {
    if (isForbiddenProductText(`${draft.titulo || ''} ${serializedDescription}`)) {
  ...
  return (
    <div className="publish-screen pb-[calc(82px+env(safe-area-inset-bottom))]">
      <header className="publish-header pt-safe">
        ...
        <div className="min-w-0 flex-1"><h1 className="text-[18px] font-black">Vender en TuTop</h1><p className="mt-0.5 text-[10px] text-slate-500">Simple primero. Detalles solo si te sirven.</p></div>
      ...
      <main className="page-pad pt-3">
        <div className="grid grid-cols-2 gap-2 rounded-2xl bg-white/[0.035] p-1.5">
          <button onClick={() => setMode('quick')} className={`rounded-xl px-3 py-3 text-left ${mode === 'quick' ? 'bg-violet-600/25 ring-1 ring-violet-400/20' : ''}`}><strong className="block text-xs">Publicación rápida</strong><span className="mt-1 block text-[9px] text-slate-500">Foto, nombre, precio y listo.</span></button>
          <button onClick={() => setMode('detailed')} className={`rounded-xl px-3 py-3 text-left ${mode === 'detailed' ? 'bg-violet-600/25 ring-1 ring-violet-400/20' : ''}`}><strong className="block text-xs">Más detalles</strong><span className="mt-1 block text-[9px] text-slate-500">Campos que cambian según lo que vendes.</span></button>
        </div>

        <div className="mt-2 flex items-center justify-between text-[9px] text-slate-600"><span className="inline-flex items-center gap-1"><Save className="h-3 w-3" />{savedAt || 'El borrador se guarda en este dispositivo'}</span><span>{visibleQuality}% completo</span></div>
        <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-violet-500 transition-all" style={{ width: `${visibleQuality}%` }} /></div>
        ...
        <section className="publish-card mt-3">
          ...
          <div className="mt-2 flex items-center gap-1.5 text-[9px] text-slate-600"><Camera className="h-3.5 w-3.5" />Puedes seleccionar varias y ordenar cuál será portada.</div>
        </section>
        ...
        <section className="publish-card mt-3">
          ...
          <div className="mt-3 grid grid-cols-[.7fr_.45fr_1.2fr] gap-2">
            ...
            <div><label className="publish-label mt-0">Categoría</label><div className="relative"><select value={draft.categoria || ''} onChange={(event) => selectCategory(event.target.value as ProductCategory)} className="publish-input appearance-none pr-9"><option value="" disabled>Elige una</option>{MARKETPLACE_CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-slate-600" /></div></div>
          </div>
        ...
        <AnimatePresence initial={false}>{mode === 'detailed' && <motion.section initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden"><div className="publish-card mt-3">
          ...
          <textarea value={draft.descripcion || ''} onChange={(event) => update('descripcion', event.target.value)} className="publish-textarea" maxLength={650} rows={4} placeholder="Cuenta lo importante sin escribir de más…" />
          ...
          {adaptiveFields.length > 0 && <div className="mt-4 rounded-2xl border border-violet-400/10 bg-violet-500/[0.04] p-3"><div className="mb-2"><strong className="text-[11px]">Detalles para {draft.categoria}</strong><p className="mt-0.5 text-[9px] text-slate-600">Solo aparecen los datos que pueden ayudar en esta categoría.</p></div><div className="grid grid-cols-2 gap-2">{adaptiveFields.map((field) => <div key={field.key}><label className="mb-1 block text-[9px] font-bold text-slate-500">{field.label}</label><input value={adaptive[field.key] || ''} onChange={(event) => setAdaptive((current) => ({ ...current, [field.key]: event.target.value.slice(0, 100) }))} inputMode={field.inputMode || 'text'} className="publish-input" placeholder={field.placeholder} /></div>)}</div></div>}
