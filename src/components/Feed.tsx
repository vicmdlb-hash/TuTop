...
import { Bell, ChevronDown, Crown, ListFilter, MapPin, Search, SlidersHorizontal, Trophy, X } from 'lucide-react';
...
export default function Feed() {
  ...
  const regularProducts = useMemo(() => {
    ...
    const list = products.filter((product) => {
      ...
      const matchesQuery = !needle || `${product.titulo} ${product.descripcion || ''} ${normalizeCategory(product.categoria)}`.toLocaleLowerCase('es-MX').includes(needle);
      return matchesFaculty && matchesStatus && matchesCategory && matchesQuery && product.precio_mxn <= priceLimit;
    });
  ...
  }, [products, currentFacultad, selectedCategoria, query, sortMode, maxPrice]);
  ...
  return (
    <div className="page-pad">
      ...
      <section className="mt-5">
        <div className="mb-2.5 flex items-center justify-between"><h2 className="section-title">Explora</h2><div className="flex gap-1"><button onClick={() => setSearchOpen((value) => !value)} className="icon-button" aria-label="Buscar"><Search className="h-5 w-5" /></button><button onClick={() => setFiltersOpen((value) => !value)} className={`icon-button ${filtersOpen ? 'text-violet-300' : ''}`} aria-label="Filtros"><SlidersHorizontal className="h-5 w-5" /></button></div></div>
        ...
        <AnimatePresence>{filtersOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-3 overflow-hidden"><div className="filter-panel"><div><label>Ordenar</label><select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)}><option value="recent">Más recientes</option><option value="price-low">Precio: menor a mayor</option><option value="price-high">Precio: mayor a menor</option></select></div><div><label>Precio máximo</label><input value={maxPrice} onChange={(event) => setMaxPrice(event.target.value)} inputMode="numeric" type="number" min="0" placeholder="Sin límite" /></div><button onClick={() => { setSortMode('recent'); setMaxPrice(''); }}><ListFilter />Limpiar</button></div></motion.div>}</AnimatePresence>
