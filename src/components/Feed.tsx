...
type BrowseScope = 'for-you' | 'campus' | 'institution' | 'city' | 'national';

const scopeOptions: Array<{ id: BrowseScope; label: string; hint: string; icon: ReactNode }> = [
  { id: 'for-you', label: 'Para ti', hint: 'Relevancia + cercanía', icon: <Sparkles /> },
  { id: 'campus', label: 'Mi campus', hint: 'Lo más cercano', icon: <MapPin /> },
  ...
  { id: 'national', label: 'Todo México', hint: 'Sólo alcance nacional', icon: <Globe2 /> },
];
...
function matchesBrowseScope(product: Product, scope: BrowseScope, user: User, legacyFaculty: string) {
...
export default function Feed() {
  ...
  const [browseScope, setBrowseScope] = useState<BrowseScope>('for-you');
  ...
  }, [products, favorites, onboardingInterests.join('|'), recentIds.join('|')]);
  ...
  const regularProducts = useMemo(() => {
    ...
    let list = products.filter((product) => {
      ...
      return matchesBrowseScope(product, browseScope, user, currentFacultad) && matchesStatus && matchesCategory && matchesQuery && matchesFavorite && matchesVerified && matchesNegotiable && matchesDelivery && product.precio_mxn <= priceLimit;
    });
  ...
  }, [products, currentFacultad, selectedCategoria, query, sortMode, maxPrice, favoritesOnly, verifiedOnly, negotiableOnly, deliveryOnly, favorites, browseScope, user, interestCategories]);
  ...
  const recentProducts = useMemo(() => recentIds.map((id) => products.find((product) => product.id === id)).filter((product): product is Product => Boolean(product && product.estado === 'Activo' && visibleByPublicationScope(product, user, currentFacultad))).slice(0, 4), [products, user, currentFacultad, recentIds.join('|')]);

  const clearFilters = () => {
    setSortMode('relevant'); setMaxPrice(''); setFavoritesOnly(false); setVerifiedOnly(false); setNegotiableOnly(false); setDeliveryOnly(false);
  };
  ...
  const saveCurrentSearch = async () => {
    ...
    if (!clean) return;
    const visibility: ListingVisibilityScope = browseScope === 'for-you' ? 'campus' : browseScope === 'institution' ? 'institution' : browseScope === 'city' ? 'city' : browseScope === 'national' ? 'national' : 'campus';
    const payload = {
      query: clean,
      category: selectedCategoria === 'Todas' ? undefined : normalizeCategory(selectedCategoria),
      max_price_mxn: maxPrice ? Number(maxPrice) : undefined,
      institution_id: userInstitution(user),
      campus_id: userCampus(user),
  ...
  return (
    <div className="page-pad">
      <header className="pt-safe flex items-center justify-between pb-4">
        <div><div className="wordmark"><span>Tu</span><span>Top</span></div><p className="mt-0.5 text-[11px] text-muted">Tu red universitaria para comprar, vender y encontrar.</p></div>
      ...
      <section className="rounded-2xl border border-violet-400/10 bg-violet-500/[0.04] p-2.5">
        <div className="mb-2 flex items-center gap-2 px-1"><MapPin className="h-3.5 w-3.5 text-violet-300" /><p className="min-w-0 flex-1 truncate text-[9px] text-slate-500">Explorando desde <strong className="text-slate-300">{identityLabel}</strong></p></div>
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">{scopeOptions.map((scope) => <button key={scope.id} onClick={() => setBrowseScope(scope.id)} className={`min-w-[116px] rounded-xl px-3 py-2 text-left transition ${browseScope === scope.id ? 'bg-violet-600/25 text-violet-100 ring-1 ring-violet-400/20' : 'bg-white/[0.025] text-slate-500'}`}><strong className="flex items-center gap-1.5 text-[10px] [&>svg]:h-3.5 [&>svg]:w-3.5">{scope.icon}{scope.label}</strong><span className="mt-0.5 block text-[8px] opacity-70">{scope.hint}</span></button>)}</div>
      </section>
