...
export default function Feed() {
  ...
  }, [scopedProducts, currentFacultad, selectedCategoria, query, sortMode, maxPrice, favoritesOnly, verifiedOnly, negotiableOnly, deliveryOnly, favorites, browseScope, user, interestCategories]);
  ...
  return (
    <div className="page-pad">
      ...
      <section className="rounded-2xl border border-violet-400/10 bg-violet-500/[0.04] p-2.5"><div className="mb-2 flex items-center gap-2 px-1"><MapPin className="h-3.5 w-3.5 text-violet-300" /><p className="min-w-0 flex-1 truncate text-[9px] text-slate-500">Explorando desde <strong className="text-slate-300">{identityLabel}</strong></p></div><div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">{scopeOptions.map((scope) => <button key={scope.id} onClick={() => setBrowseScope(scope.id)} className={`min-w-[116px] rounded-xl px-3 py-2 text-left transition ${browseScope === scope.id ? 'bg-violet-600/25 text-violet-100 ring-1 ring-violet-400/20' : 'bg-white/[0.025] text-slate-500'}`}><strong className="flex items-center gap-1.5 text-[10px] [&>svg]:h-3.5 [&>svg]:w-3.5">{scope.icon}{scope.label}</strong><span className="mt-0.5 block text-[8px] opacity-70">{scope.hint}</span></button>)}</div></section>
