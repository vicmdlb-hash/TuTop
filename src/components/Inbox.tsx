...
type InboxFilter = 'Todos' | 'Compras' | 'Ventas' | 'Pendientes';
...
export default function Inbox() {
  ...
  const visibleChats = useMemo(() => chats.filter((chat) => {
    const roleMatches = filter === 'Todos'
      || (filter === 'Compras' && chat.comprador_id === user.id)
      || (filter === 'Ventas' && chat.vendedor_id === user.id)
      || (filter === 'Pendientes' && chat.vendedor_id === user.id && (chat.sin_leer || 0) > 0);
    ...
    const aUnread = a.sin_leer || 0;
    const bUnread = b.sin_leer || 0;
    if (filter === 'Pendientes' && aUnread !== bUnread) return bUnread - aUnread;
  ...
  return (
    <div className="page-pad pt-safe">
      ...
      <section className="grid grid-cols-3 gap-2 pb-3">
        ...
        <button onClick={() => setFilter('Pendientes')} className={`rounded-2xl border p-3 text-left ${filter === 'Pendientes' ? 'border-emerald-400/20 bg-emerald-500/10' : 'border-white/5 bg-white/[0.025]'}`}><CircleDollarSign className="h-4 w-4 text-emerald-300" /><strong className="mt-2 block text-lg">{counters.Pendientes}</strong><span className="text-[9px] text-muted">Por responder</span></button>
      </section>
      ...
      <div className="segmented-control">{(['Todos', 'Compras', 'Ventas', 'Pendientes'] as const).map((value) => <button key={value} onClick={() => setFilter(value)} className={filter === value ? 'active' : ''}>{value}</button>)}</div>
