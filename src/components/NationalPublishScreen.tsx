...
import { cleanTitle, detectCategory, extractPrice, improveDescription, MARKETPLACE_CATEGORIES } from '../lib/productAssistant';
import { identityFor, safeMeetingPointsFor, VISIBILITY_SCOPES } from '../lib/universityNetwork';
...
export default function NationalPublishScreen() {
  ...
  const applyTopi = () => {
    ...
    const inferredPrice = price || (extractPrice(text) ? String(extractPrice(text)) : '');
    ...
    setMessage(inferredPrice
      ? 'Topi preparó el borrador. Revisa los datos y publica cuando quieras.'
      : 'Topi preparó título, categoría y descripción. Sólo falta que confirmes el precio.');
  };
  ...
  return <div className="publish-screen pb-[calc(82px+env(safe-area-inset-bottom))]">
    ...
    <main className="page-pad space-y-3 pt-3">
      <section className="publish-card border-violet-400/15 bg-violet-500/[0.05]"><div className="flex items-center gap-2"><WandSparkles className="h-4 w-4 text-violet-300" /><div><strong className="text-xs">Topi te ayuda a publicar</strong><p className="mt-0.5 text-[9px] text-slate-500">Escribe una sola frase. No necesitas llenar todo desde cero.</p></div></div><textarea className="publish-textarea mt-3" rows={3} value={assistantText} onChange={(e) => setAssistantText(e.target.value.slice(0, 700))} placeholder="Ej. Vendo audífonos Sony en buen estado, $2,500, entrego en Campus Ribereña." /><button type="button" onClick={applyTopi} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 py-3 text-xs font-black text-white"><Sparkles className="h-4 w-4" />Topi, prepara mi anuncio</button></section>
      ...
      {advancedOpen && <>
        ...
        <section className="publish-card"><p className="eyebrow">ALCANCE Y ENTREGA</p><div className="mt-2 grid gap-2">{VISIBILITY_SCOPES.map((item) => <button key={item.id} onClick={() => setScope(item.id)} className={`rounded-xl p-3 text-left ${scope === item.id ? 'bg-violet-500/10 ring-1 ring-violet-400/20' : 'bg-white/[0.02]'}`}><strong className="text-[10px]">{item.label}</strong><p className="text-[9px] text-slate-600">{item.hint}</p></button>)}</div><div className="mt-3 flex flex-wrap gap-2">{DELIVERY.map((item) => <button key={item.id} onClick={() => toggleDelivery(item.id)} className={`filter-chip ${deliveryMethods.includes(item.id) ? 'filter-chip-active' : ''}`}>{item.label}</button>)}</div>{scope === 'national' && <p className="mt-2 rounded-xl bg-sky-500/[0.05] px-3 py-2 text-[9px] text-sky-200/80">Para Todo México, selecciona <strong>Paquetería</strong> como forma de entrega.</p>}{safePoints.length > 0 && <select className="publish-input mt-3" value={meetingPointId} onChange={(e) => setMeetingPointId(e.target.value)}><option value="">Punto a coordinar</option>{safePoints.map((point) => <option key={point.id} value={point.id}>{point.name}</option>)}</select>}</section>
      </>}
